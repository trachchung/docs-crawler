...
## Inference Providers[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#inference-providers "Inference Providers的直接链接")
...
For the official API-key path, see the dedicated [Google Gemini guide](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/guides/google-gemini).
In the `model:` config section, you can use either `default:` or `model:` as the key name for your model ID. Both `model: { default: my-model }` and `model: { model: my-model }` work identically.
### Google Gemini via OAuth (`google-gemini-cli`)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#google-gemini-via-oauth-google-gemini-cli "google-gemini-via-oauth-google-gemini-cli的直接链接")
...
### Anthropic (Native)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#anthropic-native "Anthropic \(Native\)的直接链接")
...
### GitHub Copilot[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#github-copilot "GitHub Copilot的直接链接")
...
### xAI (Grok) — Responses API + Prompt Caching[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#xai-grok--responses-api--prompt-caching "xAI \(Grok\) — Responses API + Prompt Caching的直接链接")
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `hermes tools` → Voice & TTS, or see the [Voice & TTS](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#text-to-speech) page for config.
### NovitaAI[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#novitaai "NovitaAI的直接链接")
...
### Ollama Cloud — Managed Ollama Models, OAuth + API Key[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#ollama-cloud--managed-ollama-models-oauth--api-key "Ollama Cloud — Managed Ollama Models, OAuth + API Key的直接链接")
...
### AWS Bedrock[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#aws-bedrock "AWS Bedrock的直接链接")
...
See the [AWS Bedrock guide](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/guides/aws-bedrock) for a walkthrough of IAM setup, region selection, and cross-region inference.
### Qwen Portal (OAuth)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#qwen-portal-oauth "Qwen Portal \(OAuth\)的直接链接")
...
### Alibaba Coding Plan[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#alibaba-coding-plan "Alibaba Coding Plan的直接链接")
...
### MiniMax (OAuth)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#minimax-oauth "MiniMax \(OAuth\)的直接链接")
...
### GMI Cloud[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#gmi-cloud "GMI Cloud的直接链接")
Open and reasoning models via [GMI Cloud](https://www.gmicloud.ai/) — OpenAI-compatible API, API key authentication.

```
# GMI Cloudhermes chat --provider gmi --model deepseek-ai/DeepSeek-R1# Requires: GMI_API_KEY in ~/.hermes/.env
```

Or set it permanently in `config.yaml`:

```
model:provider:"gmi"default:"deepseek-ai/DeepSeek-R1"
```

The base URL can be overridden with `GMI_BASE_URL` (default: `https://api.gmi-serving.com/v1`).
### StepFun[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#stepfun "StepFun的直接链接")
...
### Hugging Face Inference Providers[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#hugging-face-inference-providers "Hugging Face Inference Providers的直接链接")
...
```
# Use any available modelhermes chat --provider huggingface --model Qwen/Qwen3-235B-A22B-Thinking-2507# Requires: HF_TOKEN in ~/.hermes/.env# Short aliashermes chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```

Or set it permanently in `config.yaml`:

```
model:provider:"huggingface"default:"Qwen/Qwen3-235B-A22B-Thinking-2507"
```
...
## Custom & Self-Hosted LLM Providers[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#custom--self-hosted-llm-providers "Custom & Self-Hosted LLM Providers的直接链接")
Hermes Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Hermes at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
### General Setup[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#general-setup "General Setup的直接链接")
...
```
# In ~/.hermes/config.yamlmodel:default: your-model-nameprovider: custombase_url: http://localhost:8000/v1api_key: your-key-or-leave-empty-for-local
```
...
### Switching Models with `/model`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#switching-models-with-model "switching-models-with-model的直接链接")
...
### Ollama — Local Models, Zero Config[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#ollama--local-models-zero-config "Ollama — Local Models, Zero Config的直接链接")
...
```
model:default: qwen2.5-coder:32bprovider: custombase_url: http://localhost:11434/v1context_length:32768# See warning below
```
...
### vLLM — High-Performance GPU Inference[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#vllm--high-performance-gpu-inference "vLLM — High-Performance GPU Inference的直接链接")
...
```
pip install vllmvllm serve meta-llama/Llama-3.1-70B-Instruct \--port8000\  --max-model-len 65536\  --tensor-parallel-size 2\  --enable-auto-tool-choice \  --tool-call-parser hermes
```
...
### SGLang — Fast Serving with RadixAttention[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#sglang--fast-serving-with-radixattention "SGLang — Fast Serving with RadixAttention的直接链接")
...
```
pip install"sglang[all]"python -m sglang.launch_server \--model meta-llama/Llama-3.1-70B-Instruct \--port30000\  --context-length 65536\--tp2\  --tool-call-parser qwen
```
...
### llama.cpp / llama-server — CPU & Metal Inference[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#llamacpp--llama-server--cpu--metal-inference "llama.cpp / llama-server — CPU & Metal Inference的直接链接")
...
```
# Build and start llama-servercmake -B build && cmake --build build --config Release./build/bin/llama-server \--jinja-fa\-c32768\-ngl99\-m models/qwen2.5-coder-32b-instruct-Q4_K_M.gguf \--port8080--host0.0.0.0
```
...
### LM Studio — Desktop App with Local Models[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#lm-studio--desktop-app-with-local-models "LM Studio — Desktop App with Local Models的直接链接")
...
```
lms server start                        # Starts on port 1234lms load qwen2.5-coder --context-length 32768
```
...
### WSL2 Networking (Windows Users)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#wsl2-networking-windows-users "WSL2 Networking \(Windows Users\)的直接链接")
...
#### Option 1: Mirrored Networking Mode (Recommended)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#option-1-mirrored-networking-mode-recommended "Option 1: Mirrored Networking Mode \(Recommended\)的直接链接")
...
#### Option 2: Use the Windows Host IP (Windows 10 / older builds)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#option-2-use-the-windows-host-ip-windows-10--older-builds "Option 2: Use the Windows Host IP \(Windows 10 / older builds\)的直接链接")
...
```
model:default: qwen2.5-coder:32bprovider: custombase_url: http://172.29.192.1:11434/v1   # Windows host IP, not localhost
```

The host IP can change on WSL2 restart. You can grab it dynamically in your shell:

```
exportWSL_HOST=$(ip route show |grep-i default |awk'{ print $3 }')echo"Windows host at: $WSL_HOST"curl http://$WSL_HOST:11434/v1/models   # Test Ollama
```

Or use your machine's mDNS name (requires `libnss-mdns` in WSL2):

```
sudoaptinstall libnss-mdnscurl http://$(hostname).local:11434/v1/models
```

#### Server Bind Address (Required for NAT Mode)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#server-bind-address-required-for-nat-mode "Server Bind Address \(Required for NAT Mode\)的直接链接")
...
#### Windows Firewall[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#windows-firewall "Windows Firewall的直接链接")
...
#### Quick Verification[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#quick-verification "Quick Verification的直接链接")
...
### Troubleshooting Local Models[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#troubleshooting-local-models "Troubleshooting Local Models的直接链接")
These issues affect **all** local inference servers when used with Hermes.
#### "Connection refused" from WSL2 to a Windows-hosted model server[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#connection-refused-from-wsl2-to-a-windows-hosted-model-server ""Connection refused" from WSL2 to a Windows-hosted model server的直接链接")
If you're running Hermes inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#wsl2-networking-windows-users) above for the fix.
#### Tool calls appear as text instead of executing[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#tool-calls-appear-as-text-instead-of-executing "Tool calls appear as text instead of executing的直接链接")
...
#### Model seems to forget context or give incoherent responses[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#model-seems-to-forget-context-or-give-incoherent-responses "Model seems to forget context or give incoherent responses的直接链接")
...
#### "Context limit: 2048 tokens" at startup[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#context-limit-2048-tokens-at-startup ""Context limit: 2048 tokens" at startup的直接链接")
...
```
model:default: your-modelprovider: custombase_url: http://localhost:11434/v1context_length:32768
```

#### Responses get cut off mid-sentence[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#responses-get-cut-off-mid-sentence "Responses get cut off mid-sentence的直接链接")
**Possible causes:**
  ...
  2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/user-guide/configuration#context-compression) in Hermes.


### LiteLLM Proxy — Multi-Provider Gateway[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#litellm-proxy--multi-provider-gateway "LiteLLM Proxy — Multi-Provider Gateway的直接链接")
...
```
# Install and startpip install"litellm[proxy]"litellm --model anthropic/claude-sonnet-4 --port4000# Or with a config file for multiple models:litellm --config litellm_config.yaml --port4000
```
...
```
model_list:-model_name:"best"litellm_params:model: anthropic/claude-sonnet-4api_key: sk-ant-...-model_name:"best"litellm_params:model: openai/gpt-4oapi_key: sk-...router_settings:routing_strategy:"latency-based-routing"
```

### ClawRouter — Cost-Optimized Routing[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#clawrouter--cost-optimized-routing "ClawRouter — Cost-Optimized Routing的直接链接")
...
### Other Compatible Providers[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#other-compatible-providers "Other Compatible Providers的直接链接")
...
| `https://api.groq.com/openai/v1`  | Ultra-fast inference  |  
| `https://api.deepseek.com/v1`  | DeepSeek models  |  
| [Fireworks AI](https://fireworks.ai)  | `https://api.fireworks.ai/inference/v1`  | Fast open model hosting  |  
| `https://api.gmi-serving.com/v1`  | Managed OpenAI-compatible inference  |  
| `https://api.cerebras.ai/v1`  | Wafer-scale chip inference  |  
| [Mistral AI](https://mistral.ai)  | `https://api.mistral.ai/v1`  | Mistral models  |  
| `https://api.openai.com/v1`  | Direct OpenAI access  |  
| [Azure OpenAI](https://azure.microsoft.com)  | `https://YOUR.openai.azure.com/`  | Enterprise OpenAI  |  
| `http://localhost:8080/v1`  | Self-hosted, multi-model  |  
| `http://localhost:1337/v1`  | Desktop app with local models  |  
Configure any of these with `hermes model` → Custom endpoint, or in `config.yaml`:

```
model:default: meta-llama/Llama-3.1-70B-Instruct-Turboprovider: custombase_url: https://api.together.xyz/v1api_key: your-together-key
```

### Context Length Detection[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#context-length-detection "Context Length Detection的直接链接")
...
```
custom_providers:-name:"My Local LLM"base_url:"http://localhost:11434/v1"models:qwen3.5:27b:context_length:32768deepseek-r1:70b:context_length:65536
```
...
### Named Custom Providers[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#named-custom-providers "Named Custom Providers的直接链接")
...
### Cookbook: Together AI, Groq, Perplexity[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#cookbook-together-ai-groq-perplexity "Cookbook: Together AI, Groq, Perplexity的直接链接")
The cloud providers listed in [Other Compatible Providers](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.hermes/config.yaml` and the matching API key goes in `~/.hermes/.env`.
#### Together AI[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#together-ai "Together AI的直接链接")
...
#### Groq[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#groq "Groq的直接链接")
Ultra-fast inference (~500 tok/s on Llama-3.3-70B). Small catalog but strong for latency-sensitive interactive use.

```
# ~/.hermes/config.yamlcustom_providers:-name: groqbase_url: https://api.groq.com/openai/v1key_env: GROQ_API_KEYmodel:default: llama-3.3-70b-versatileprovider: custom:groq
```
...
#### Perplexity[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#perplexity "Perplexity的直接链接")
...
```
# ~/.hermes/config.yamlcustom_providers:-name: perplexitybase_url: https://api.perplexity.aikey_env: PERPLEXITY_API_KEYmodel:default: sonarprovider: custom:perplexity
```
...
#### Multiple providers in one config[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#multiple-providers-in-one-config "Multiple providers in one config的直接链接")
The three recipes compose — use all of them together and switch per turn with `/model custom:<name>:<model>`:

```
custom_providers:-name: togetherbase_url: https://api.together.xyz/v1key_env: TOGETHER_API_KEY-name: groqbase_url: https://api.groq.com/openai/v1key_env: GROQ_API_KEY-name: perplexitybase_url: https://api.perplexity.aikey_env: PERPLEXITY_API_KEYmodel:default: MiniMaxAI/MiniMax-M2.7provider: custom:together      # boot to Together; switch freely after
```
...
### Choosing the Right Setup[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#choosing-the-right-setup "Choosing the Right Setup的直接链接")  
...
## Optional API Keys[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#optional-api-keys "Optional API Keys的直接链接")  
| Feature  | Provider  | Env Variable  |  
| --- | --- | --- |  
| Web scraping  |  `FIRECRAWL_API_KEY`, `FIRECRAWL_API_URL`  |  
| Browser automation  | [Browserbase](https://browserbase.com/)  |  `BROWSERBASE_API_KEY`, `BROWSERBASE_PROJECT_ID`  |  
| Image generation  | `FAL_KEY`  |  
| Premium TTS voices  | [ElevenLabs](https://elevenlabs.io/)  | `ELEVENLABS_API_KEY`  |  
| OpenAI TTS + voice transcription  | `VOICE_TOOLS_OPENAI_KEY`  |  
| Mistral TTS + voice transcription  | `MISTRAL_API_KEY`  |  
| RL Training  |  [Tinker](https://tinker-console.thinkingmachines.ai/) + [WandB](https://wandb.ai/)  |  `TINKER_API_KEY`, `WANDB_API_KEY`  |  
| Cross-session user modeling  | `HONCHO_API_KEY`  |  
| Semantic long-term memory  | [Supermemory](https://supermemory.ai)  | `SUPERMEMORY_API_KEY`  |  
### Self-Hosting Firecrawl[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#self-hosting-firecrawl "Self-Hosting Firecrawl的直接链接")
...
## OpenRouter Provider Routing[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#openrouter-provider-routing "OpenRouter Provider Routing的直接链接")
...
## OpenRouter Pareto Code Router[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#openrouter-pareto-code-router "OpenRouter Pareto Code Router的直接链接")
...
```
model:provider: openroutermodel: openrouter/pareto-codeopenrouter:min_coding_score:0.65# 0.0–1.0; higher = stronger (more expensive) coders. Default 0.65.
```
...
```
fallback_providers:-provider: openroutermodel: anthropic/claude-sonnet-4-provider: anthropicmodel: claude-sonnet-4# base_url: http://localhost:8000/v1    # optional, for custom endpoints# api_mode: chat_completions           # optional override
```
...
Fallback is configured exclusively through `config.yaml` — or interactively via `hermes fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/user-guide/features/fallback-providers).
## See Also[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#see-also "See Also的直接链接")
  * [Configuration](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/user-guide/configuration) — General configuration (directory structure, config precedence, terminal backends, memory, compression, and more)
  * [Environment Variables](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/reference/environment-variables) — Complete reference of all environment variables


  * [Inference Providers](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#inference-providers)
    * [Google Gemini via OAuth (`google-gemini-cli`)](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#google-gemini-via-oauth-google-gemini-cli)
    * [Two Commands for Model Management](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#two-commands-for-model-management)
    * [Anthropic (Native)](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#anthropic-native)
    * [GitHub Copilot](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#github-copilot)
    * [First-Class API-Key Providers](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#first-class-api-key-providers)
    * [xAI (Grok) — Responses API + Prompt Caching](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#xai-grok--responses-api--prompt-caching)
    * [Ollama Cloud — Managed Ollama Models, OAuth + API Key](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#ollama-cloud--managed-ollama-models-oauth--api-key)
    * [AWS Bedrock](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#aws-bedrock)
    * [Qwen Portal (OAuth)](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#qwen-portal-oauth)
    * [Alibaba Coding Plan](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#alibaba-coding-plan)
    * [MiniMax (OAuth)](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#minimax-oauth)
    * [Hugging Face Inference Providers](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#hugging-face-inference-providers)
  * [Custom & Self-Hosted LLM Providers](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#custom--self-hosted-llm-providers)
    * [General Setup](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#general-setup)
    * [Switching Models with `/model`](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#switching-models-with-model)
    * [Ollama — Local Models, Zero Config](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#ollama--local-models-zero-config)
    * [vLLM — High-Performance GPU Inference](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#vllm--high-performance-gpu-inference)
    * [SGLang — Fast Serving with RadixAttention](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#sglang--fast-serving-with-radixattention)
    * [llama.cpp / llama-server — CPU & Metal Inference](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#llamacpp--llama-server--cpu--metal-inference)
    * [LM Studio — Desktop App with Local Models](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#lm-studio--desktop-app-with-local-models)
    * [WSL2 Networking (Windows Users)](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#wsl2-networking-windows-users)
    * [Troubleshooting Local Models](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#troubleshooting-local-models)
    * [LiteLLM Proxy — Multi-Provider Gateway](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#litellm-proxy--multi-provider-gateway)
    * [ClawRouter — Cost-Optimized Routing](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#clawrouter--cost-optimized-routing)
    * [Other Compatible Providers](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#other-compatible-providers)
    * [Context Length Detection](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#context-length-detection)
    * [Named Custom Providers](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#named-custom-providers)
    * [Cookbook: Together AI, Groq, Perplexity](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#cookbook-together-ai-groq-perplexity)
    * [Choosing the Right Setup](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#choosing-the-right-setup)
  * [Optional API Keys](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#optional-api-keys)
    * [Self-Hosting Firecrawl](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#self-hosting-firecrawl)
  * [OpenRouter Provider Routing](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#openrouter-provider-routing)
  * [OpenRouter Pareto Code Router](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#openrouter-pareto-code-router)
  * [Fallback Providers](https://hermes-agent.nousresearch.com/docs/zh-Hans/integrations/providers#fallback-providers)
