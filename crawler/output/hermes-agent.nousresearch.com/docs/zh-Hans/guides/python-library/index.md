<!-- Source: https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library -->
...
## Installation[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#installation "Installation的直接链接")
...
## Basic Usage[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#basic-usage "Basic Usage的直接链接")
The simplest way to use Hermes is the `chat()` method — pass a message, get a string back:

```
from run_agent import AIAgentagent = AIAgent(    model="anthropic/claude-sonnet-4",    quiet_mode=True,response = agent.chat("What is the capital of France?")print(response)
```
...
## Full Conversation Control[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#full-conversation-control "Full Conversation Control的直接链接")
...
## Configuring Tools[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#configuring-tools "Configuring Tools的直接链接")
Control which toolsets the agent has access to using `enabled_toolsets` or `disabled_toolsets`:

```
# Only enable web tools (browsing, search)agent = AIAgent(    model="anthropic/claude-sonnet-4",    enabled_toolsets=["web"],    quiet_mode=True,# Enable everything except terminal accessagent = AIAgent(    model="anthropic/claude-sonnet-4",    disabled_toolsets=["terminal"],    quiet_mode=True,
```
...
## Multi-turn Conversations[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#multi-turn-conversations "Multi-turn Conversations的直接链接")
...
## Saving Trajectories[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#saving-trajectories "Saving Trajectories的直接链接")
Enable trajectory saving to capture conversations in ShareGPT format — useful for generating training data or debugging:

```
agent = AIAgent(    model="anthropic/claude-sonnet-4",    save_trajectories=True,    quiet_mode=True,agent.chat("Write a Python function to sort a list")# Saves to trajectory_samples.jsonl in ShareGPT format
```

Each conversation is appended as a single JSONL line, making it easy to collect datasets from automated runs.
## Custom System Prompts[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#custom-system-prompts "Custom System Prompts的直接链接")
...
## Batch Processing[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#batch-processing "Batch Processing的直接链接")
...
## Integration Examples[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#integration-examples "Integration Examples的直接链接")
### FastAPI Endpoint[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#fastapi-endpoint "FastAPI Endpoint的直接链接")

```
from fastapi import FastAPIfrom pydantic import BaseModelfrom run_agent import AIAgentapp = FastAPI()classChatRequest(BaseModel):    message:str    model:str="anthropic/claude-sonnet-4"@app.post("/chat")asyncdefchat(request: ChatRequest):    agent = AIAgent(        model=request.model,        quiet_mode=True,        skip_context_files=True,        skip_memory=True,    response = agent.chat(request.message)return{"response": response}
```

### Discord Bot[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#discord-bot "Discord Bot的直接链接")

```
import discordfrom run_agent import AIAgentclient = discord.Client(intents=discord.Intents.default())@client.eventasyncdefon_message(message):if message.author == client.user:returnif message.content.startswith("!hermes "):        query = message.content[8:]        agent = AIAgent(            model="anthropic/claude-sonnet-4",            quiet_mode=True,            skip_context_files=True,            skip_memory=True,            platform="discord",        response = agent.chat(query)await message.channel.send(response[:2000])client.run("YOUR_DISCORD_TOKEN")
```

### CI/CD Pipeline Step[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#cicd-pipeline-step "CI/CD Pipeline Step的直接链接")
...
## Key Constructor Parameters[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#key-constructor-parameters "Key Constructor Parameters的直接链接")  
...
## Important Notes[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#important-notes "Important Notes的直接链接")
  ...
  * [Installation](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#installation)
  * [Basic Usage](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#basic-usage)
  * [Full Conversation Control](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#full-conversation-control)
  * [Configuring Tools](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#configuring-tools)
  * [Multi-turn Conversations](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#multi-turn-conversations)
  * [Saving Trajectories](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#saving-trajectories)
  * [Custom System Prompts](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#custom-system-prompts)
  * [Batch Processing](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#batch-processing)
  * [Integration Examples](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#integration-examples)
    * [FastAPI Endpoint](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#fastapi-endpoint)
    * [Discord Bot](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#discord-bot)
    * [CI/CD Pipeline Step](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#cicd-pipeline-step)
  * [Key Constructor Parameters](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#key-constructor-parameters)
  * [Important Notes](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/python-library#important-notes)
