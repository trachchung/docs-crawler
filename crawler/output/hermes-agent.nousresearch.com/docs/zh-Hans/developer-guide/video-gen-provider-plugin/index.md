<!-- Source: https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin -->
...
Video-gen mirrors [Image Generation Provider Plugins](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/developer-guide/image-gen-provider-plugin) almost line-for-line — if you've built an image-gen backend, you already know the shape. The main differences: a `capabilities()` method advertising modalities/aspect-ratios/durations, and a routing convention (pass `image_url` to use image-to-video, omit it to use text-to-video — the provider picks the right endpoint internally).
## The unified surface (one tool, two modalities)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#the-unified-surface-one-tool-two-modalities "The unified surface \(one tool, two modalities\)的直接链接")
...
## How discovery works[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#how-discovery-works "How discovery works的直接链接")
...
## Directory structure[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#directory-structure "Directory structure的直接链接")

```
plugins/video_gen/my-backend/├── __init__.py      # VideoGenProvider subclass + register()└── plugin.yaml      # Manifest with kind: backend
```

## The VideoGenProvider ABC[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#the-videogenprovider-abc "The VideoGenProvider ABC的直接链接")
Subclass `agent.video_gen_provider.VideoGenProvider`. Required: `name` property and `generate()` method.

```
# plugins/video_gen/my-backend/__init__.pyfrom typing import Any, Dict, List, Optionalimport osfrom agent.video_gen_provider import(    VideoGenProvider,    error_response,    success_response,classMyVideoGenProvider(VideoGenProvider):@propertydefname(self)->str:return"my-backend"@propertydefdisplay_name(self)->str:return"My Backend"defis_available(self)->bool:returnbool(os.environ.get("MY_API_KEY"))deflist_models(self)-> List[Dict[str, Any]]:# Each entry is a model FAMILY — a name the user picks once.# Your provider's generate() routes within the family based on# whether image_url was passed.return["id":"fast","display":"Fast","speed":"~30s","strengths":"Cheapest tier","price":"$0.05/s","modalities":["text","image"],# advisorydefdefault_model(self)-> Optional[str]:return"fast"defcapabilities(self)-> Dict[str, Any]:return{"modalities":["text","image"],"aspect_ratios":["16:9","9:16"],"resolutions":["720p","1080p"],"min_duration":1,"max_duration":10,"supports_audio":False,"supports_negative_prompt":True,"max_reference_images":0,defget_setup_schema(self)-> Dict[str, Any]:return{"name":"My Backend","badge":"paid","tag":"Short description shown in `hermes tools`","env_vars":["key":"MY_API_KEY","prompt":"My Backend API key","url":"https://mybackend.example.com/keys",defgenerate(        self,        prompt:str,        model: Optional[str]=None,        image_url: Optional[str]=None,        reference_image_urls: Optional[List[str]]=None,        duration: Optional[int]=None,        aspect_ratio:str="16:9",        resolution:str="720p",        negative_prompt: Optional[str]=None,        audio: Optional[bool]=None,        seed: Optional[int]=None,**kwargs: Any,# always ignore unknown kwargs for forward-compat)-> Dict[str, Any]:# ROUTE: image_url presence picks the endpoint.if image_url:            endpoint ="my-backend/image-to-video"            modality_used ="image"else:            endpoint ="my-backend/text-to-video"            modality_used ="text"# ... call your API ...return success_response(            video="https://your-cdn/output.mp4",            model=model or"fast",            prompt=prompt,            modality=modality_used,            aspect_ratio=aspect_ratio,            duration=duration or5,            provider=self.name,defregister(ctx)->None:    ctx.register_video_gen_provider(MyVideoGenProvider())
```

## The plugin manifest[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#the-plugin-manifest "The plugin manifest的直接链接")
...
## The `video_generate` schema[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#the-video_generate-schema "the-video_generate-schema的直接链接")
...
## Model families and endpoint routing (the FAL pattern)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#model-families-and-endpoint-routing-the-fal-pattern "Model families and endpoint routing \(the FAL pattern\)的直接链接")
...
## Selection precedence[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#selection-precedence "Selection precedence的直接链接")
...
## Response shape[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#response-shape "Response shape的直接链接")
...
## Where to save artifacts[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#where-to-save-artifacts "Where to save artifacts的直接链接")
...
## Testing[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#testing "Testing的直接链接")
Drop a smoke test under `tests/plugins/video_gen/test_<name>_plugin.py`. The xAI and FAL tests show the pattern — register, verify catalog, exercise routing both with and without `image_url`, assert clean error responses on missing auth.
  * [The unified surface (one tool, two modalities)](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#the-unified-surface-one-tool-two-modalities)
  * [How discovery works](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#how-discovery-works)
  * [Directory structure](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#directory-structure)
  * [The VideoGenProvider ABC](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#the-videogenprovider-abc)
  * [The plugin manifest](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#the-plugin-manifest)
  * [The `video_generate` schema](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#the-video_generate-schema)
  * [Model families and endpoint routing (the FAL pattern)](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#model-families-and-endpoint-routing-the-fal-pattern)
  * [Selection precedence](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#selection-precedence)
  * [Response shape](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#response-shape)
  * [Where to save artifacts](https://hermes-agent.nousresearch.com/docs/zh-Hans/developer-guide/video-gen-provider-plugin#where-to-save-artifacts)
