<!-- Source: https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables -->

本页总览
All variables go in `~/.hermes/.env`. You can also set them with `hermes config set VAR value`.
## LLM Providers[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#llm-providers "LLM Providers的直接链接")  
...
## Provider Auth (OAuth)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#provider-auth-oauth "Provider Auth \(OAuth\)的直接链接")
...
| Variable  | Description  |  
| --- | --- |  
| `HERMES_INFERENCE_PROVIDER`  | Override provider selection: `auto`, `custom`, `openrouter`, `nous`, `openai-codex`, `copilot`, `copilot-acp`, `anthropic`, `huggingface`, `novita`, `gemini`, `zai`, `kimi-coding`, `kimi-coding-cn`, `minimax`, `minimax-cn`, `minimax-oauth` (browser OAuth login — no API key required; see [MiniMax OAuth guide](https://hermes-agent.nousresearch.com/docs/zh-Hans/guides/minimax-oauth)), `kilocode`, `xiaomi`, `arcee`, `gmi`, `stepfun`, `alibaba`, `alibaba-coding-plan` (alias `alibaba_coding`), `deepseek`, `nvidia`, `ollama-cloud`, `xai` (alias `grok`), `google-gemini-cli`, `qwen-oauth`, `bedrock`, `opencode-zen`, `opencode-go`, `ai-gateway`, `tencent-tokenhub` (default: `auto`)  |  
...
## Tool APIs[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#tool-apis "Tool APIs的直接链接")  
| Variable  | Description  |  
| --- | --- |  
...
### Langfuse Observability[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#langfuse-observability "Langfuse Observability的直接链接")
Environment variables for the bundled [`observability/langfuse`](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/user-guide/features/built-in-plugins#observabilitylangfuse) plugin. Set these with `hermes tools → Langfuse Observability` or manually in `~/.hermes/.env`. The plugin must also be enabled (`hermes plugins enable observability/langfuse`) before any of these take effect.  
| Variable  | Description  |  
| --- | --- |  
...
### Nous Tool Gateway[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#nous-tool-gateway "Nous Tool Gateway的直接链接")
These variables configure the [Tool Gateway](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/user-guide/features/tool-gateway) for paid Nous subscribers or self-hosted gateway deployments. Most users don't need to set these — the gateway is configured automatically via `hermes model` or `hermes tools`.  
| Variable  | Description  |  
| --- | --- |  
...
## Terminal Backend[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#terminal-backend "Terminal Backend的直接链接")  
| Variable  | Description  |  
| --- | --- |  
...
## SSH Backend[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#ssh-backend "SSH Backend的直接链接")  
| Variable  | Description  |  
| --- | --- |  
...
## Container Resources (Docker, Singularity, Modal, Daytona)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#container-resources-docker-singularity-modal-daytona "Container Resources \(Docker, Singularity, Modal, Daytona\)的直接链接")  
| Variable  | Description  |  
| --- | --- |  
...
## Persistent Shell[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#persistent-shell "Persistent Shell的直接链接")  
| Variable  | Description  |  
| --- | --- |  
...
## Messaging[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#messaging "Messaging的直接链接")  
| Variable  | Description  |  
| --- | --- |  
...
| `DISCORD_ALLOW_MENTION_EVERYONE`  | Allow the bot to ping `@everyone`/`@here` (default: `false`). See [Mention Control](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/discord#mention-control).  |  
...
| `FEISHU_ALLOW_BOTS`  |  `none` (default) / `mentions` / `all` — accept inbound messages from other bots. See [bot-to-bot messaging](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/messaging/feishu#bot-to-bot-messaging)  |  
...
### Microsoft Graph (Teams Meetings)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#microsoft-graph-teams-meetings "Microsoft Graph \(Teams Meetings\)的直接链接")
App-only credentials for the Microsoft Graph REST client used by the upcoming Teams meeting summary pipeline. See [Register a Microsoft Graph application](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/guides/microsoft-graph-app-registration) for the Azure portal walkthrough and the exact API permissions required.  
| Variable  | Description  |  
| --- | --- |  
...
### Microsoft Graph Webhook Listener[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#microsoft-graph-webhook-listener "Microsoft Graph Webhook Listener的直接链接")
Inbound change-notification listener for Graph events (Teams meetings, calendar, chat, etc.). See [Microsoft Graph Webhook Listener](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/user-guide/messaging/msgraph-webhook) for setup and security hardening.  
| Variable  | Description  |  
| --- | --- |  
...
### Teams Meeting Summary Delivery[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#teams-meeting-summary-delivery "Teams Meeting Summary Delivery的直接链接")
Only used when the [`teams_pipeline` plugin](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/user-guide/messaging/msgraph-webhook) is enabled. Settings are also configurable under `platforms.teams.extra` in `config.yaml` — env vars take priority when both are set. See [Microsoft Teams → Meeting Summary Delivery](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/user-guide/messaging/teams#meeting-summary-delivery-teams-meeting-pipeline).  
| Variable  | Description  |  
| --- | --- |  
...
### LINE Messaging API[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#line-messaging-api "LINE Messaging API的直接链接")
Used by the bundled LINE platform plugin (`plugins/platforms/line/`). See [Messaging Gateway → LINE](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/user-guide/messaging/line) for full setup.  
| Variable  | Description  |  
| --- | --- |  
...
### Advanced Messaging Tuning[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#advanced-messaging-tuning "Advanced Messaging Tuning的直接链接")
...
## Agent Behavior[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#agent-behavior "Agent Behavior的直接链接")  
| Variable  | Description  |  
| --- | --- |  
...
| `CODEX_HOME`  | When [Codex app-server runtime](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/codex-app-server-runtime) is enabled, override the directory Codex CLI reads its config + auth from (default: `~/.codex`). Hermes' migration writes the managed block to `<CODEX_HOME>/config.toml`.  |  
...
## Interface[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#interface "Interface的直接链接")  
| Variable  | Description  |  
| --- | --- |  
| `HERMES_TUI`  | Launch the [TUI](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/tui) instead of the classic CLI when set to `1`. Equivalent to passing `--tui`.  |  
...
## Session Settings[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#session-settings "Session Settings的直接链接")  
| Variable  | Description  |  
| --- | --- |  
...
## Context Compression (config.yaml only)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#context-compression-configyaml-only "Context Compression \(config.yaml only\)的直接链接")
...
```
compression:enabled:truethreshold:0.50target_ratio:0.20# fraction of threshold to preserve as recent tailprotect_last_n:20# minimum recent messages to keep uncompressed
```
...
## Auxiliary Task Overrides[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#auxiliary-task-overrides "Auxiliary Task Overrides的直接链接")  
| Variable  | Description  |  
| --- | --- |  
...
## Fallback Providers (config.yaml only)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#fallback-providers-configyaml-only "Fallback Providers \(config.yaml only\)的直接链接")
...
See [Fallback Providers](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/user-guide/features/fallback-providers) for full details.
## Provider Routing (config.yaml only)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#provider-routing-configyaml-only "Provider Routing \(config.yaml only\)的直接链接")
...
Use `hermes config set` to set environment variables — it automatically saves them to the right file (`.env` for secrets, `config.yaml` for everything else).
  * [LLM Providers](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#llm-providers)
  * [Provider Auth (OAuth)](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#provider-auth-oauth)
  * [Tool APIs](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#tool-apis)
    * [Langfuse Observability](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#langfuse-observability)
    * [Nous Tool Gateway](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#nous-tool-gateway)
  * [Terminal Backend](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#terminal-backend)
  * [SSH Backend](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#ssh-backend)
  * [Container Resources (Docker, Singularity, Modal, Daytona)](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#container-resources-docker-singularity-modal-daytona)
  * [Persistent Shell](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#persistent-shell)
  * [Messaging](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#messaging)
    * [Microsoft Graph (Teams Meetings)](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#microsoft-graph-teams-meetings)
    * [Microsoft Graph Webhook Listener](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#microsoft-graph-webhook-listener)
    * [Teams Meeting Summary Delivery](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#teams-meeting-summary-delivery)
    * [LINE Messaging API](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#line-messaging-api)
    * [Advanced Messaging Tuning](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#advanced-messaging-tuning)
  * [Agent Behavior](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#agent-behavior)
  * [Session Settings](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#session-settings)
  * [Context Compression (config.yaml only)](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#context-compression-configyaml-only)
  * [Auxiliary Task Overrides](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#auxiliary-task-overrides)
  * [Fallback Providers (config.yaml only)](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#fallback-providers-configyaml-only)
  * [Provider Routing (config.yaml only)](https://hermes-agent.nousresearch.com/docs/zh-Hans/reference/environment-variables#provider-routing-configyaml-only)
