<!-- Source: https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts -->

本页总览
Hermes Agent supports both text-to-speech output and voice message transcription across all messaging platforms.
If you have a paid [Nous Portal](https://portal.nousresearch.com) subscription, OpenAI TTS is available through the **[Tool Gateway](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tool-gateway.md)** without a separate OpenAI API key. Run `hermes model` or `hermes tools` to enable it.
## Text-to-Speech[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#text-to-speech "Text-to-Speech的直接链接")
...
### Platform Delivery[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#platform-delivery "Platform Delivery的直接链接")  
...
### Configuration[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#configuration "Configuration的直接链接")
...
### Input length limits[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#input-length-limits "Input length limits的直接链接")
...
### Telegram Voice Bubbles & ffmpeg[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#telegram-voice-bubbles--ffmpeg "Telegram Voice Bubbles & ffmpeg的直接链接")
...
```
# Ubuntu/Debiansudoaptinstall ffmpeg# macOSbrew install ffmpeg# Fedorasudo dnf install ffmpeg
```
...
### xAI Custom Voices (voice cloning)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#xai-custom-voices-voice-cloning "xAI Custom Voices \(voice cloning\)的直接链接")
...
### Piper (local, 44 languages)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#piper-local-44-languages "Piper \(local, 44 languages\)的直接链接")
...
### Custom command providers[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#custom-command-providers "Custom command providers的直接链接")
...
```
tts:provider: voxcpm                 # pick any name under tts.providersproviders:voxcpm:type: commandcommand:"voxcpm --ref ~/voice.wav --text-file {input_path} --out {output_path}"output_format: mp3timeout:180voice_compatible:true# try to deliver as a Telegram voice bubblemlx-kokoro:type: commandcommand:"python -m mlx_kokoro --in {input_path} --out {output_path} --voice {voice}"voice: af_skyoutput_format: wavpiper-custom:# native Piper also supports custom .onnx via tts.piper.voicetype: commandcommand:"piper -m /path/to/custom.onnx -f {output_path} < {input_path}"output_format: wav
```

#### Example: Doubao (Chinese seed-tts-2.0)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#example-doubao-chinese-seed-tts-20 "Example: Doubao \(Chinese seed-tts-2.0\)的直接链接")
...
```
pip install doubao-speechexportVOLCENGINE_APP_ID="your-app-id"exportVOLCENGINE_ACCESS_TOKEN="your-access-token"
```


```
tts:provider: doubaoproviders:doubao:type: commandcommand:"doubao-speech say --text-file {input_path} --out {output_path}"output_format: mp3max_text_length:1024timeout:30
```

Credentials come from your shell environment (`VOLCENGINE_APP_ID` / `VOLCENGINE_ACCESS_TOKEN`) or `~/.doubao-speech/config.yaml`. Pick a voice by adding `--voice zh-female-warm` (or any other alias from `doubao-speech list-voices`) to the command. `doubao-speech` also bundles streaming ASR — see the [STT section below](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#example-doubao--volcengine-asr) for Hermes integration. Source and full docs: [github.com/Hypnus-Yuan/doubao-speech](https://github.com/Hypnus-Yuan/doubao-speech).
#### Placeholders[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#placeholders "Placeholders的直接链接")
...
#### Optional keys[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#optional-keys "Optional keys的直接链接")  
| Key  | Default  | Meaning  |  
| --- | --- | --- |  
...
#### Behavior notes[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#behavior-notes "Behavior notes的直接链接")
...
#### Security[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#security "Security的直接链接")
...
## Voice Message Transcription (STT)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#voice-message-transcription-stt "Voice Message Transcription \(STT\)的直接链接")
...
### Configuration[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#configuration-1 "Configuration的直接链接")

```
# In ~/.hermes/config.yamlstt:provider:"local"# "local" | "groq" | "openai" | "mistral" | "xai"local:model:"base"# tiny, base, small, medium, large-v3openai:model:"whisper-1"# whisper-1, gpt-4o-mini-transcribe, gpt-4o-transcribemistral:model:"voxtral-mini-latest"# voxtral-mini-latest, voxtral-mini-2602xai:model:"grok-stt"# xAI Grok STT
```

### Provider Details[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#provider-details "Provider Details的直接链接")
...
#### Example: Doubao / Volcengine ASR[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#example-doubao--volcengine-asr "Example: Doubao / Volcengine ASR的直接链接")
If you use [`doubao-speech`](https://pypi.org/project/doubao-speech/) for Doubao TTS (see [above](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#example-doubao-chinese-seed-tts-20)), the same package handles speech-to-text via the local-command STT surface:

```
pip install doubao-speechexportVOLCENGINE_APP_ID="your-app-id"exportVOLCENGINE_ACCESS_TOKEN="your-access-token"exportHERMES_LOCAL_STT_COMMAND='doubao-speech transcribe {input_path} --out {output_dir}/transcript.txt'
```
...
### Fallback Behavior[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#fallback-behavior "Fallback Behavior的直接链接")
If your configured provider isn't available, Hermes automatically falls back:
  ...
  * [Text-to-Speech](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#text-to-speech)
    * [Platform Delivery](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#platform-delivery)
    * [Configuration](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#configuration)
    * [Input length limits](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#input-length-limits)
    * [Telegram Voice Bubbles & ffmpeg](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#telegram-voice-bubbles--ffmpeg)
    * [xAI Custom Voices (voice cloning)](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#xai-custom-voices-voice-cloning)
    * [Piper (local, 44 languages)](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#piper-local-44-languages)
    * [Custom command providers](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#custom-command-providers)
  * [Voice Message Transcription (STT)](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#voice-message-transcription-stt)
    * [Configuration](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#configuration-1)
    * [Provider Details](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#provider-details)
    * [Fallback Behavior](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/tts#fallback-behavior)
