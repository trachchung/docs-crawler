<!-- Source: https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks -->

本页总览
Hermes has three hook systems that run custom code at key lifecycle points:  
| System  | Registered via  | Runs in  | Use case  |  
| --- | --- | --- | --- |  
| **[Gateway hooks](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#gateway-event-hooks)**  |  `HOOK.yaml` + `handler.py` in `~/.hermes/hooks/`  | Gateway only  | Logging, alerts, webhooks  |  
| **[Plugin hooks](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#plugin-hooks)**  |  `ctx.register_hook()` in a [plugin](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/user-guide/features/plugins)  | CLI + Gateway  | Tool interception, metrics, guardrails  |  
| **[Shell hooks](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#shell-hooks)**  |  `hooks:` block in `~/.hermes/config.yaml` pointing at shell scripts  | CLI + Gateway  | Drop-in scripts for blocking, auto-formatting, context injection  |  
All three systems are non-blocking — errors in any hook are caught and logged, never crashing the agent.
## Gateway Event Hooks[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#gateway-event-hooks "Gateway Event Hooks的直接链接")
...
### Creating a Hook[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#creating-a-hook "Creating a Hook的直接链接")
...
#### HOOK.yaml[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#hookyaml "HOOK.yaml的直接链接")
...
#### handler.py[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#handlerpy "handler.py的直接链接")

```
import jsonfrom datetime import datetimefrom pathlib import PathLOG_FILE = Path.home()/".hermes"/"hooks"/"my-hook"/"activity.log"asyncdefhandle(event_type:str, context:dict):"""Called for each subscribed event. Must be named 'handle'."""    entry ={"timestamp": datetime.now().isoformat(),"event": event_type,**context,withopen(LOG_FILE,"a")as f:.write(json.dumps(entry)+"\n")
```
...
### Available Events[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#available-events "Available Events的直接链接")  
...
#### Wildcard Matching[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#wildcard-matching "Wildcard Matching的直接链接")
Handlers registered for `command:*` fire for any `command:` event (`command:model`, `command:reset`, etc.). Monitor all slash commands with a single subscription.
### Examples[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#examples "Examples的直接链接")
#### Telegram Alert on Long Tasks[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#telegram-alert-on-long-tasks "Telegram Alert on Long Tasks的直接链接")
...
```
# ~/.hermes/hooks/long-task-alert/handler.pyimport osimport httpxTHRESHOLD =10BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN")CHAT_ID = os.getenv("TELEGRAM_HOME_CHANNEL")asyncdefhandle(event_type:str, context:dict):    iteration = context.get("iteration",0)if iteration == THRESHOLD and BOT_TOKEN and CHAT_ID:        tools =", ".join(context.get("tool_names",[]))        text =f"⚠️ Agent has been running for {iteration} steps. Last tools: {tools}"asyncwith httpx.AsyncClient()as client:await client.post(f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage",                json={"chat_id": CHAT_ID,"text": text},
```

#### Command Usage Logger[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#command-usage-logger "Command Usage Logger的直接链接")
...
```
# ~/.hermes/hooks/command-logger/handler.pyimport jsonfrom datetime import datetimefrom pathlib import PathLOG = Path.home()/".hermes"/"logs"/"command_usage.jsonl"defhandle(event_type:str, context:dict):    LOG.parent.mkdir(parents=True, exist_ok=True)    entry ={"ts": datetime.now().isoformat(),"command": context.get("command"),"args": context.get("args"),"platform": context.get("platform"),"user": context.get("user_id"),withopen(LOG,"a")as f:.write(json.dumps(entry)+"\n")
```

#### Session Start Webhook[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#session-start-webhook "Session Start Webhook的直接链接")
...
```
# ~/.hermes/hooks/session-webhook/handler.pyimport httpxWEBHOOK_URL ="https://your-service.example.com/hermes-events"asyncdefhandle(event_type:str, context:dict):asyncwith httpx.AsyncClient()as client:await client.post(WEBHOOK_URL, json={"event": event_type,**context,}, timeout=5)
```

### Tutorial: BOOT.md — Run a Startup Checklist on Every Gateway Boot[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#tutorial-bootmd--run-a-startup-checklist-on-every-gateway-boot "Tutorial: BOOT.md — Run a Startup Checklist on Every Gateway Boot的直接链接")
...
#### What we're building[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#what-were-building "What we're building的直接链接")
...
#### Step 1: Write your checklist[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#step-1-write-your-checklist "Step 1: Write your checklist的直接链接")
...
#### Step 2: Create the hook[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#step-2-create-the-hook "Step 2: Create the hook的直接链接")
...
```
"""Run ~/.hermes/BOOT.md on every gateway startup."""import loggingimport threadingfrom pathlib import Pathlogger = logging.getLogger("hooks.boot-md")BOOT_FILE = Path.home()/".hermes"/"BOOT.md"def_build_prompt(content:str)->str:return("You are running a startup boot checklist. Follow the instructions ""below exactly.\n\n""---\n"f"{content}\n""---\n\n""Execute each instruction. Use the send_message tool to deliver any ""messages to platforms like Discord or Slack.\n""If nothing needs attention and there is nothing to report, reply ""with ONLY: [SILENT]"def_run_boot_agent(content:str)->None:"""Spawn a one-shot agent and execute the checklist.    Uses the gateway's resolved model and runtime credentials so this works    against custom endpoints, aggregators, and OAuth-based providers alike.    """try:from gateway.run import _resolve_gateway_model, _resolve_runtime_agent_kwargsfrom run_agent import AIAgent        agent = AIAgent(            model=_resolve_gateway_model(),**_resolve_runtime_agent_kwargs(),            platform="gateway",            quiet_mode=True,            skip_context_files=True,            skip_memory=True,            max_iterations=20,        result = agent.run_conversation(_build_prompt(content))        response = result.get("final_response","")if response and"[SILENT]"notin response:            logger.info("boot-md completed: %s", response[:200])else:            logger.info("boot-md completed (nothing to report)")except Exception as e:        logger.error("boot-md agent failed: %s", e)asyncdefhandle(event_type:str, context:dict)->None:ifnot BOOT_FILE.exists():return    content = BOOT_FILE.read_text(encoding="utf-8").strip()ifnot content:return    logger.info("Running BOOT.md (%d chars)",len(content))# Background thread so gateway startup isn't blocked on a full agent turn.    thread = threading.Thread(        target=_run_boot_agent,        args=(content,),        name="boot-md",        daemon=True,    thread.start()
```
...
#### Step 3: Test it[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#step-3-test-it "Step 3: Test it的直接链接")
Restart the gateway:
...
#### Extending the pattern[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#extending-the-pattern "Extending the pattern的直接链接")
...
#### Why this isn't a built-in[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#why-this-isnt-a-built-in "Why this isn't a built-in的直接链接")
...
### How It Works[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#how-it-works "How It Works的直接链接")
...
Gateway hooks only fire in the **gateway** (Telegram, Discord, Slack, WhatsApp, Teams). The CLI does not load gateway hooks. For hooks that work everywhere, use [plugin hooks](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#plugin-hooks).
## Plugin Hooks[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#plugin-hooks "Plugin Hooks的直接链接")
[Plugins](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/user-guide/features/plugins) can register hooks that fire in **both CLI and gateway** sessions. These are registered programmatically via `ctx.register_hook()` in your plugin's `register()` function.

```
defregister(ctx):    ctx.register_hook("pre_tool_call", my_tool_observer)    ctx.register_hook("post_tool_call", my_tool_logger)    ctx.register_hook("pre_llm_call", my_memory_callback)    ctx.register_hook("post_llm_call", my_sync_callback)    ctx.register_hook("on_session_start", my_init_callback)    ctx.register_hook("on_session_end", my_cleanup_callback)
```

**General rules for all hooks:**
  * Callbacks receive **keyword arguments**. Always accept `**kwargs` for forward compatibility — new parameters may be added in future versions without breaking your plugin.
  * If a callback **crashes** , it's logged and skipped. Other hooks and the agent continue normally. A misbehaving plugin can never break the agent.
  * Two hooks' return values affect behavior: [`pre_tool_call`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#pre_tool_call) can **block** the tool, and [`pre_llm_call`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#pre_llm_call) can **inject context** into the LLM call. All other hooks are fire-and-forget observers.


### Quick reference[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#quick-reference "Quick reference的直接链接")  
| Hook  | Fires when  | Returns  |  
| --- | --- | --- |  
| [`pre_tool_call`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#pre_tool_call)  | Before any tool executes  |  `{"action": "block", "message": str}` to veto the call  |  
| [`post_tool_call`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#post_tool_call)  | After any tool returns  | ignored  |  
| [`pre_llm_call`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#pre_llm_call)  | Once per turn, before the tool-calling loop  |  `{"context": str}` to prepend context to the user message  |  
| [`post_llm_call`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#post_llm_call)  | Once per turn, after the tool-calling loop  | ignored  |  
| [`on_session_start`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#on_session_start)  | New session created (first turn only)  | ignored  |  
| [`on_session_end`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#on_session_end)  | Session ends  | ignored  |  
| [`on_session_finalize`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#on_session_finalize)  | CLI/gateway tears down an active session (flush, save, stats)  | ignored  |  
| [`on_session_reset`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#on_session_reset)  | Gateway swaps in a fresh session key (e.g. `/new`, `/reset`)  | ignored  |  
| [`subagent_stop`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#subagent_stop)  | A `delegate_task` child has exited  | ignored  |  
| [`pre_gateway_dispatch`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#pre_gateway_dispatch)  | Gateway received a user message, before auth + dispatch  |  `{"action": "skip" | "rewrite" | "allow", ...}` to influence flow  |  
| [`pre_approval_request`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#pre_approval_request)  | Dangerous command needs user approval, before the prompt/notification is sent  | ignored  |  
| [`post_approval_response`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#post_approval_response)  | User responded to an approval prompt (or it timed out)  | ignored  |  
| [`transform_tool_result`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#transform_tool_result)  | After any tool returns, before the result is handed back to the model  |  `str` to replace the result, `None` to leave unchanged  |  
| [`transform_terminal_output`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#transform_terminal_output)  | Inside the `terminal` tool, before truncation/ANSI-strip/redact  |  `str` to replace the raw output, `None` to leave unchanged  |  
| [`transform_llm_output`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#transform_llm_output)  | After the tool-calling loop completes, before the final response is delivered  |  `str` to replace the response text, `None`/empty to leave unchanged  |  
###  `pre_tool_call`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#pre_tool_call "pre_tool_call的直接链接")
...
```
import json, loggingfrom datetime import datetimelogger = logging.getLogger(__name__)defaudit_tool_call(tool_name, args, task_id,**kwargs):    logger.info("TOOL_CALL session=%s tool=%s args=%s",                task_id, tool_name, json.dumps(args)[:200])defregister(ctx):    ctx.register_hook("pre_tool_call", audit_tool_call)
```

**Example — warn on dangerous tools:**

```
DANGEROUS ={"terminal","write_file","patch"}defwarn_dangerous(tool_name,**kwargs):if tool_name in DANGEROUS:print(f"⚠ Executing potentially dangerous tool: {tool_name}")defregister(ctx):    ctx.register_hook("pre_tool_call", warn_dangerous)
```

###  `post_tool_call`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#post_tool_call "post_tool_call的直接链接")
Fires **immediately after** every tool execution returns.
**Callback signature:**

```
defmy_callback(tool_name:str, args:dict, result:str, task_id:str,                duration_ms:int,**kwargs):
```
...
```
from collections import Counter, defaultdictimport json_tool_counts = Counter()_error_counts = Counter()_latency_ms = defaultdict(list)deftrack_metrics(tool_name, result, duration_ms=0,**kwargs):    _tool_counts[tool_name]+=1    _latency_ms[tool_name].append(duration_ms)try:        parsed = json.loads(result)if"error"in parsed:            _error_counts[tool_name]+=1except(json.JSONDecodeError, TypeError):passdefregister(ctx):    ctx.register_hook("post_tool_call", track_metrics)
```

###  `pre_llm_call`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#pre_llm_call "pre_llm_call的直接链接")
...
```
defmy_callback(session_id:str, user_message:str, conversation_history:list,                is_first_turn:bool, model:str, platform:str,**kwargs):
```
...
```
import httpxMEMORY_API ="https://your-memory-api.example.com"defrecall(session_id, user_message, is_first_turn,**kwargs):try:        resp = httpx.post(f"{MEMORY_API}/recall", json={"session_id": session_id,"query": user_message,}, timeout=3)        memories = resp.json().get("results",[])ifnot memories:returnNone        text ="Recalled context:\n"+"\n".join(f"- {m['text']}"for m in memories)return{"context": text}except Exception:returnNonedefregister(ctx):    ctx.register_hook("pre_llm_call", recall)
```
...
###  `post_llm_call`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#post_llm_call "post_llm_call的直接链接")
...
```
defmy_callback(session_id:str, user_message:str, assistant_response:str,                conversation_history:list, model:str, platform:str,**kwargs):
```
...
```
import httpxMEMORY_API ="https://your-memory-api.example.com"defsync_memory(session_id, user_message, assistant_response,**kwargs):try:        httpx.post(f"{MEMORY_API}/store", json={"session_id": session_id,"user": user_message,"assistant": assistant_response,}, timeout=5)except Exception:pass# best-effortdefregister(ctx):    ctx.register_hook("post_llm_call", sync_memory)
```

**Example — track response lengths:**

```
import logginglogger = logging.getLogger(__name__)deflog_response_length(session_id, assistant_response, model,**kwargs):    logger.info("RESPONSE session=%s model=%s chars=%d",                session_id, model,len(assistant_response or""))defregister(ctx):    ctx.register_hook("post_llm_call", log_response_length)
```

###  `on_session_start`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#on_session_start "on_session_start的直接链接")
...
```
_session_caches ={}definit_session(session_id, model, platform,**kwargs):    _session_caches[session_id]={"model": model,"platform": platform,"tool_calls":0,"started":__import__("datetime").datetime.now().isoformat(),defregister(ctx):    ctx.register_hook("on_session_start", init_session)
```

###  `on_session_end`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#on_session_end "on_session_end的直接链接")
...
```
defmy_callback(session_id:str, completed:bool, interrupted:bool,                model:str, platform:str,**kwargs):
```
...
```
import time, logginglogger = logging.getLogger(__name__)_start_times ={}defon_start(session_id,**kwargs):    _start_times[session_id]= time.time()defon_end(session_id, completed, interrupted,**kwargs):    start = _start_times.pop(session_id,None)if start:        duration = time.time()- start        logger.info("SESSION_DURATION session=%s seconds=%.1f completed=%s interrupted=%s",                     session_id, duration, completed, interrupted)defregister(ctx):    ctx.register_hook("on_session_start", on_start)    ctx.register_hook("on_session_end", on_end)
```

###  `on_session_finalize`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#on_session_finalize "on_session_finalize的直接链接")
...
###  `on_session_reset`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#on_session_reset "on_session_reset的直接链接")
...
See the **[Build a Plugin guide](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/guides/build-a-hermes-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
###  `subagent_stop`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#subagent_stop "subagent_stop的直接链接")
...
```
defmy_callback(parent_session_id:str, child_role:str|None,                child_summary:str|None, child_status:str,                duration_ms:int,**kwargs):
```
...
```
import logginglogger = logging.getLogger(__name__)deflog_subagent(parent_session_id, child_role, child_status, duration_ms,**kwargs):    logger.info("SUBAGENT parent=%s role=%s status=%s duration_ms=%d",        parent_session_id, child_role, child_status, duration_ms,defregister(ctx):    ctx.register_hook("subagent_stop", log_subagent)
```
...
###  `pre_gateway_dispatch`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#pre_gateway_dispatch "pre_gateway_dispatch的直接链接")
...
```
defdeny_unauthorized_dms(event,**kwargs):    src = event.sourceif src.chat_type =="dm"andnot _is_approved_user(src.user_id):return{"action":"skip","reason":"unauthorized-dm"}returnNonedefregister(ctx):    ctx.register_hook("pre_gateway_dispatch", deny_unauthorized_dms)
```

**Example — rewrite an ambient-message buffer into a single prompt on mention:**

```
_buffers ={}defbuffer_or_rewrite(event,**kwargs):    key =(event.source.platform, event.source.chat_id)    buf = _buffers.setdefault(key,[])if _bot_mentioned(event.text):        combined ="\n".join(buf +[event.text])        buf.clear()return{"action":"rewrite","text": combined}    buf.append(event.text)return{"action":"skip","reason":"ambient-buffered"}defregister(ctx):    ctx.register_hook("pre_gateway_dispatch", buffer_or_rewrite)
```

###  `pre_approval_request`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#pre_approval_request "pre_approval_request的直接链接")
...
```
defmy_callback(    command:str,    description:str,    pattern_key:str,    pattern_keys:list[str],    session_key:str,    surface:str,**kwargs,
```
...
**Return value:** ignored. Hooks here are observer-only; they cannot veto or pre-answer the approval. Use [`pre_tool_call`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#pre_tool_call) to block a tool before it reaches the approval system.
...
```
import subprocessdefnotify_approval(command, description, session_key,**kwargs):    title ="Hermes needs approval"    body =f"{description}: {command[:80]}"    subprocess.Popen(["osascript","-e",f'display notification "{body}" with title "{title}"',defregister(ctx):    ctx.register_hook("pre_approval_request", notify_approval)
```

###  `post_approval_response`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#post_approval_response "post_approval_response的直接链接")
Fires **after** the user responds to an approval prompt (or the prompt times out).
**Callback signature:**

```
defmy_callback(    command:str,    description:str,    pattern_key:str,    pattern_keys:list[str],    session_key:str,    surface:str,    choice:str,**kwargs,
```
...
###  `transform_tool_result`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#transform_tool_result "transform_tool_result的直接链接")
...
```
defmy_callback(    tool_name:str,    arguments:dict,    result:str,    task_id:str|None,**kwargs,)->str|None:
```
...
```
import reSECRET = re.compile(r"sk-[A-Za-z0-9]{32,}")defredact_secrets(tool_name, result,**kwargs):if SECRET.search(result):return SECRET.sub("[REDACTED]", result)returnNonedefregister(ctx):    ctx.register_hook("transform_tool_result", redact_secrets)
```
...
###  `transform_terminal_output`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#transform_terminal_output "transform_terminal_output的直接链接")
...
```
defmy_callback(    command:str,    output:str,    exit_code:int,    cwd:str,    task_id:str|None,**kwargs,)->str|None:
```
...
```
defsummarize_find(command, output,**kwargs):if command.startswith("find ")andlen(output)>50_000:        lines = output.count("\n")        head ="\n".join(output.splitlines()[:40])returnf"{head}\n\n[summary: {lines} paths total, showing first 40]"returnNonedefregister(ctx):    ctx.register_hook("transform_terminal_output", summarize_find)
```

Pairs well with `transform_tool_result` (which covers every other tool).
###  `transform_llm_output`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#transform_llm_output "transform_llm_output的直接链接")
...
```
defmy_callback(    response_text:str,    session_id:str,    model:str,    platform:str,**kwargs,)->str|None:
```
...
```
import os, redefspongebob(response_text,**kwargs):if os.environ.get("SPONGEBOB_MODE")!="on":returnNone# pass through unchangedreturn re.sub(r"!","!! Tartar sauce!", response_text)defregister(ctx):    ctx.register_hook("transform_llm_output", spongebob)
```
...
## Shell Hooks[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#shell-hooks "Shell Hooks的直接链接")
...
Use shell hooks when you want a drop-in, single-file script (Bash, Python, anything with a shebang) to:
  ...
  * **Inject context into the next LLM turn** — prepend `git status` output, the current weekday, or retrieved documents to the user message (see [`pre_llm_call`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#pre_llm_call)).
...
### Comparison at a glance[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#comparison-at-a-glance "Comparison at a glance的直接链接")  
| Dimension  | Shell hooks  | [Plugin hooks](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#plugin-hooks)  | [Gateway hooks](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#gateway-event-hooks)  |  
| --- | --- | --- | --- |  
...
### Configuration schema[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#configuration-schema "Configuration schema的直接链接")

```
hooks:<event_name>:# Must be in VALID_HOOKS-matcher:"<regex>"# Optional; used for pre/post_tool_call onlycommand:"<shell command>"# Required; runs via shlex.split, shell=Falsetimeout: <seconds># Optional; default 60, capped at 300hooks_auto_accept:false# See "Consent model" below
```

Event names must be one of the [plugin hook events](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#plugin-hooks); typos produce a "Did you mean X?" warning and are skipped. Unknown keys inside a single entry are ignored; missing `command` is a skip-with-warning. `timeout > 300` is clamped with a warning.
### JSON wire protocol[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#json-wire-protocol "JSON wire protocol的直接链接")
...
### Worked examples[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#worked-examples "Worked examples的直接链接")
#### 1. Auto-format Python files after every write[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#1-auto-format-python-files-after-every-write "1. Auto-format Python files after every write的直接链接")
...
#### 2. Block destructive `terminal` commands[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#2-block-destructive-terminal-commands "2-block-destructive-terminal-commands的直接链接")
...
```
#!/usr/bin/env bash# ~/.hermes/agent-hooks/block-rm-rf.shpayload="$(cat -)"cmd=$(echo"$payload"| jq -r'.tool_input.command // empty')ifecho"$cmd"|grep-qE'rm[[:space:]]+-rf?[[:space:]]+/';thenprintf'{"decision": "block", "reason": "blocked: rm -rf / is not permitted"}\n'elseprintf'{}\n'
```

#### 3. Inject `git status` into every turn (Claude-Code `UserPromptSubmit` equivalent)[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#3-inject-git-status-into-every-turn-claude-code-userpromptsubmit-equivalent "3-inject-git-status-into-every-turn-claude-code-userpromptsubmit-equivalent的直接链接")
...
```
#!/usr/bin/env bash# ~/.hermes/agent-hooks/inject-cwd-context.shcat - >/dev/null   # discard stdin payloadifstatus=$(git status --porcelain2>/dev/null)&&[[-n"$status"]];then  jq --null-input --arg s "$status"\'{context: ("Uncommitted changes in cwd:\n" + $s)}'elseprintf'{}\n'
```
...
#### 4. Log every subagent completion[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#4-log-every-subagent-completion "4. Log every subagent completion的直接链接")
...
### Consent model[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#consent-model "Consent model的直接链接")
...
### The `hermes hooks` CLI[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#the-hermes-hooks-cli "the-hermes-hooks-cli的直接链接")  
...
### Security[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#security "Security的直接链接")
...
### Ordering and precedence[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#ordering-and-precedence "Ordering and precedence的直接链接")
Both Python plugin hooks and shell hooks flow through the same `invoke_hook()` dispatcher. Python plugins are registered first (`discover_and_load()`), shell hooks second (`register_from_config()`), so Python `pre_tool_call` block decisions take precedence in tie cases. The first valid block wins — the aggregator returns as soon as any callback produces `{"action": "block", "message": str}` with a non-empty message.
  * [Gateway Event Hooks](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#gateway-event-hooks)
    * [Creating a Hook](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#creating-a-hook)
    * [Available Events](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#available-events)
    * [Tutorial: BOOT.md — Run a Startup Checklist on Every Gateway Boot](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#tutorial-bootmd--run-a-startup-checklist-on-every-gateway-boot)
    * [How It Works](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#how-it-works)
  * [Plugin Hooks](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#plugin-hooks)
    * [Quick reference](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#quick-reference)
    * [`pre_tool_call`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#pre_tool_call)
    * [`post_tool_call`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#post_tool_call)
    * [`pre_llm_call`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#pre_llm_call)
    * [`post_llm_call`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#post_llm_call)
    * [`on_session_start`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#on_session_start)
    * [`on_session_end`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#on_session_end)
    * [`on_session_finalize`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#on_session_finalize)
    * [`on_session_reset`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#on_session_reset)
    * [`subagent_stop`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#subagent_stop)
    * [`pre_gateway_dispatch`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#pre_gateway_dispatch)
    * [`pre_approval_request`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#pre_approval_request)
    * [`post_approval_response`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#post_approval_response)
    * [`transform_tool_result`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#transform_tool_result)
    * [`transform_terminal_output`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#transform_terminal_output)
    * [`transform_llm_output`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#transform_llm_output)
  * [Shell Hooks](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#shell-hooks)
    * [Comparison at a glance](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#comparison-at-a-glance)
    * [Configuration schema](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#configuration-schema)
    * [JSON wire protocol](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#json-wire-protocol)
    * [Worked examples](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#worked-examples)
    * [Consent model](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#consent-model)
    * [The `hermes hooks` CLI](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#the-hermes-hooks-cli)
    * [Ordering and precedence](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/features/hooks#ordering-and-precedence)
