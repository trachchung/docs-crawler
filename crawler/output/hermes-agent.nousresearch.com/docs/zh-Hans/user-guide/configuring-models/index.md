<!-- Source: https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models -->
...
This page covers configuring both from the dashboard. If you prefer config files or the CLI, jump to [Alternative methods](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#alternative-methods) at the bottom.
## The Models page[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#the-models-page "The Models page的直接链接")
...
## Setting the main model[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#setting-the-main-model "Setting the main model的直接链接")
...
## Setting auxiliary models[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#setting-auxiliary-models "Setting auxiliary models的直接链接")
...
### Common override patterns[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#common-override-patterns "Common override patterns的直接链接")  
...
### Per-task override[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#per-task-override "Per-task override的直接链接")
...
### Reset all to auto[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#reset-all-to-auto "Reset all to auto的直接链接")
...
## The "Use as" shortcut[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#the-use-as-shortcut "The "Use as" shortcut的直接链接")
...
## What gets written to `config.yaml`[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#what-gets-written-to-configyaml "what-gets-written-to-configyaml的直接链接")
When you save via the dashboard, Hermes writes to `~/.hermes/config.yaml`:
**Main model:**

```
model:provider: openrouterdefault: anthropic/claude-opus-4.7base_url:''# cleared on provider switchapi_mode: chat_completions
```

**Auxiliary override (example — vision on gemini-flash):**

```
auxiliary:vision:provider: openroutermodel: google/gemini-2.5-flashbase_url:''api_key:''timeout:120extra_body:{}download_timeout:30
```

**Auxiliary on auto (default):**

```
auxiliary:compression:provider: automodel:''base_url:''# ... other fields unchanged
```

`provider: auto` with `model: ''` tells Hermes to use the main model for that task.
## When does it take effect?[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#when-does-it-take-effect "When does it take effect?的直接链接")
...
## Troubleshooting[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#troubleshooting "Troubleshooting的直接链接")
### "No authenticated providers" in the picker[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#no-authenticated-providers-in-the-picker ""No authenticated providers" in the picker的直接链接")
...
### Main model didn't change in my running chat[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#main-model-didnt-change-in-my-running-chat "Main model didn't change in my running chat的直接链接")
...
### Auxiliary override "didn't take effect"[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#auxiliary-override-didnt-take-effect "Auxiliary override "didn't take effect"的直接链接")
...
### I picked a model but Hermes switched providers on me[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#i-picked-a-model-but-hermes-switched-providers-on-me "I picked a model but Hermes switched providers on me的直接链接")
...
## Alternative methods[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#alternative-methods "Alternative methods的直接链接")
### CLI slash command[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#cli-slash-command "CLI slash command的直接链接")
...
### Custom aliases[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#custom-aliases "Custom aliases的直接链接")
Define your own short names for models you reach for often, then use `/model <alias>` in the CLI or any messaging platform:

```
# ~/.hermes/config.yamlmodel_aliases:fav:model: claude-sonnet-4.6provider: anthropicgrok:model: grok-4provider: x-ai
```
...
Then `/model fav` or `/model grok` in chat. User aliases shadow built-in short names (`sonnet`, `kimi`, `opus`, etc.). See [Custom model aliases](https://hermes-agent.nousresearch.com/docs/zh-Hans/docs/reference/slash-commands#custom-model-aliases) for the full reference.
###  `hermes model` subcommand[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#hermes-model-subcommand "hermes-model-subcommand的直接链接")
...
### Direct config edit[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#direct-config-edit "Direct config edit的直接链接")
Edit `~/.hermes/config.yaml` and restart whatever reads it. See the [Configuration reference](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuration) for the full schema.
### REST API[​](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#rest-api "REST API的直接链接")
...
The session token is injected into the dashboard HTML at startup and rotates on every server restart. Grab it from the browser devtools (`window.__HERMES_SESSION_TOKEN__`) if you're scripting against a running dashboard.
  * [The Models page](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#the-models-page)
  * [Setting the main model](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#setting-the-main-model)
  * [Setting auxiliary models](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#setting-auxiliary-models)
    * [Common override patterns](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#common-override-patterns)
    * [Per-task override](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#per-task-override)
    * [Reset all to auto](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#reset-all-to-auto)
  * [The "Use as" shortcut](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#the-use-as-shortcut)
  * [What gets written to `config.yaml`](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#what-gets-written-to-configyaml)
  * [When does it take effect?](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#when-does-it-take-effect)
  * [Troubleshooting](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#troubleshooting)
    * ["No authenticated providers" in the picker](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#no-authenticated-providers-in-the-picker)
    * [Main model didn't change in my running chat](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#main-model-didnt-change-in-my-running-chat)
    * [Auxiliary override "didn't take effect"](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#auxiliary-override-didnt-take-effect)
    * [I picked a model but Hermes switched providers on me](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#i-picked-a-model-but-hermes-switched-providers-on-me)
  * [Alternative methods](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#alternative-methods)
    * [CLI slash command](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#cli-slash-command)
    * [Custom aliases](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#custom-aliases)
    * [`hermes model` subcommand](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#hermes-model-subcommand)
    * [Direct config edit](https://hermes-agent.nousresearch.com/docs/zh-Hans/user-guide/configuring-models#direct-config-edit)
