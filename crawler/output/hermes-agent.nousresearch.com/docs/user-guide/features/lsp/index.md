...
[Skip to main content](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#__docusaurus_skipToContent_fallback)
On this page
...
## When LSP runs[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#when-lsp-runs "Direct link to When LSP runs")
...
## Supported languages[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#supported-languages "Direct link to Supported languages")  
...
## CLI[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#cli "Direct link to CLI")
...
## Configuration[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#configuration "Direct link to Configuration")
The defaults work for typical setups; nothing to set if the binaries are on PATH.

```
# config.yamllsp:# Master toggle. Disabling skips the entire subsystem — no servers# spawn, no background event loop runs.enabled:true# How long to wait for diagnostics after each write.wait_mode: document      # "document" or "full"wait_timeout:5.0# How to handle missing server binaries.#   auto    — install via npm/pip/go install into <HERMES_HOME>/lsp/bin#   manual  — only use binaries already on PATHinstall_strategy: auto# Per-server overrides (all optional).servers:pyright:disabled:falsecommand:["/abs/path/to/pyright-langserver","--stdio"]env:{PYRIGHT_LOG_LEVEL:"info"}initialization_options:python:analysis:typeCheckingMode:"strict"typescript:disabled:true# skip TS even when its extensions match
```

### Per-server keys[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#per-server-keys "Direct link to Per-server keys")
...
## Installation locations[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#installation-locations "Direct link to Installation locations")
...
## Performance characteristics[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#performance-characteristics "Direct link to Performance characteristics")
...
## Disabling[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#disabling "Direct link to Disabling")
...
## Troubleshooting[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#troubleshooting "Direct link to Troubleshooting")
...
By design, LSP only runs inside a git repository. If the project isn't yet initialized, run `git init` to enable LSP diagnostics. Otherwise the in-process syntax-only fallback applies.
  * [When LSP runs](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#when-lsp-runs)
  * [Supported languages](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#supported-languages)
  * [Configuration](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#configuration)
    * [Per-server keys](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#per-server-keys)
  * [Installation locations](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#installation-locations)
  * [Performance characteristics](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#performance-characteristics)
  * [Troubleshooting](https://hermes-agent.nousresearch.com/docs/user-guide/features/lsp#troubleshooting)
