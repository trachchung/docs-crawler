...
[Skip to main content](https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial#__docusaurus_skipToContent_fallback)
On this page
...
```
SCHEMA=$(hermes kanban create "Design auth schema"\--assignee backend-dev --tenant auth-project --priority2\--body"Design the user/session/token schema for the auth module."\--json| jq -r .id)API=$(hermes kanban create "Implement auth API endpoints"\--assignee backend-dev --tenant auth-project --priority2\--parent $SCHEMA \--body"POST /register, POST /login, POST /refresh, POST /logout."\--json| jq -r .id)hermes kanban create "Write auth integration tests"\--assignee qa-dev --tenant auth-project --priority2\--parent$API\--body"Cover happy path, wrong password, expired token, concurrent refresh."
```
...
## Structured handoff — why `summary` and `metadata` matter[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial#structured-handoff--why-summary-and-metadata-matter "Direct link to structured-handoff--why-summary-and-metadata-matter")
...
## Next steps[​](https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial#next-steps "Direct link to Next steps")
  ...
  * [The board at a glance](https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial#the-board-at-a-glance)
  * [Story 1 — Solo dev shipping a feature](https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial#story-1--solo-dev-shipping-a-feature)
  * [Story 2 — Fleet farming](https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial#story-2--fleet-farming)
  * [Story 3 — Role pipeline with retry](https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial#story-3--role-pipeline-with-retry)
  * [Story 4 — Circuit breaker and crash recovery](https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial#story-4--circuit-breaker-and-crash-recovery)
    * [Circuit breaker — permanent-looking failure](https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial#circuit-breaker--permanent-looking-failure)
    * [Crash recovery — worker dies mid-flight](https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial#crash-recovery--worker-dies-mid-flight)
  * [Structured handoff — why `summary` and `metadata` matter](https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial#structured-handoff--why-summary-and-metadata-matter)
  * [Inspecting a task currently running](https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban-tutorial#inspecting-a-task-currently-running)
