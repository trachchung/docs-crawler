...
The entrypoint script (`docker/entrypoint.sh`) bootstraps the data volume on first run:
  ...
  * Optionally launches `hermes dashboard` as a background side-process when `HERMES_DASHBOARD=1` (see [Running the dashboard](https://hermes-agent.nousresearch.com/docs/user-guide/docker#running-the-dashboard))
