...
[Skip to main content](https://docs.meshconnect.com/api-reference/balance/get-account-balance#content-area)
...
`Robinhood` `Coinbase` `Kraken` `CryptoCom` `Binance` `Gemini` `OkCoin` `KuCoin` `CexIo` `BinanceInternational` `Bitstamp` `GateIo` `Okx` `BitFlyer` `Coinlist` `Huobi` `Bitfinex` `KrakenDirect` `BinanceInternationalDirect` `BitfinexDirect` `Bybit` `Paxos` `CoinbasePrime` `BtcTurkDirect` `KuCoinDirect` `OkxOAuth` `ParibuDirect` `RobinhoodConnect` `BlockchainCom` `BitsoDirect` `BybitDirect` `ParibuOAuth` `BinanceTrDirect` `BybitDirectMobile` `Sandbox` `Uphold` `SandboxCoinbase`
Available options: 
`robinhood`, 
`eTrade`, 
`alpaca`, 
...
YesNo
[ Get aggregated portfolio Previous ](https://docs.meshconnect.com/api-reference/portfolio/get-aggregated-portfolio)[ Get aggregated portfolio fiat balances Next ](https://docs.meshconnect.com/api-reference/balance/get-aggregated-portfolio-fiat-balances)
Ctrl+I
Get account balance
cURL
